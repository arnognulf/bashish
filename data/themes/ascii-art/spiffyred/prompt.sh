...
test "x${RCS_PS1}" != x && RCS_PS1=" "${SPIFFYDULL}"${RCS_PS1}"
local i=0
local FILLY=""
local BASHISH_CWD=`_bashish_prompt_cwd $SHELLNAME "${SPIFFYDULL_NOEMBED}" "${SPIFFYNORMAL_NOEMBED}" 39`

test "x$PROMPTSTR" = x && local PROMPTSTR="${USER}${SPIFFYNORMAL}@${SPIFFYDULL}${HOSTNAME%%.*}"
...
PS1="${SPIFFYNORMAL}[${SPIFFYDULL}${PROMPTSTR} ${BASHISH_CWD}${RCS_PS1}${SPIFFYNORMAL}]${ROOT}${EMBED}${ESC}[0m${UNEMBED} "
}
_bashish_prompt
