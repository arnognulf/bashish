...
PROMPT_COMMAND="_BASHISH_PROMPT_RCS=\$(_bashish_prompt_rcs \${_BASHISH_PROMPT_RCS} ) && _bashish_promptupdate"
	...
	TITLE="${HOSTNAME%%.*}"
	test x"$PROMPTSTR" = x && local PROMPTSTR="$HOSTNAME"
	i=1
	LINECHAR=" "
	...
	NEWLINE="
"
	PS1="${UNDERLINE}${NEWLINE}${EMBED}${ESC}[A${ESC}[${COLUMNS}G${VT100LINECHAR}${LINECHAR}${ESC}[0m${ESC}[B${ESC}[0G${ESC}[${BOLD};7m${UNEMBED} "$PROMPTSTR" ${RCS_PS1}${ROOT}${EMBED}${ESC}[0m${UNEMBED} "
}
